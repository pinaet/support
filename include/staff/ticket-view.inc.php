        ...
        <td width="50%">
            <table border="0" cellspacing="0" cellpadding="4" width="100%">
                ...
                <tr>
                    ...
                    <?php
                    //select id department_id, name department_name from sup_department order by id;
                    
                    $sql             = "select id department_id, name department_name from sup_department order by id;";
                    $res             = db_query($sql);
                    $departments     = db_assoc_array($res);
                    if ($role->hasPerm(Ticket::PERM_TRANSFER)) {?>
                      <td>
                            <!-- bak
                            ...
                            <span class="inline-edit" data-placement="bottom" data-toggle="tooltip" title="<?php echo __('Transfer'); ?>">
                            <?php
                            $label    = __('Department');
                            $ticket_id= $ticket->getId();
                            $form_id  = 0;
                            $input_id = 'field_department_'.$ticket_id;
                            $field_id = $ticket_id;
                            $username = $thisstaff->getUsername();
                            $old_value= $ticket->getDeptName();
                            $ctype    = 'department';
                            echo '<select id="'.$input_id.'" style="width:190px" onchange="update_ticket_detail(\''.$label.'\','.$ticket_id.','.$form_id.',\''.$input_id.'\','.$field_id.',\''.$username.'\',\''.$old_value.'\',\''.$ctype.'\')">';
                            echo '<option value="0">'.'&mdash; ' . __('Unassigned') .  ' &mdash;'.'</option>';
                            foreach( $departments as $department ){
                                echo sprintf('<option value="%s"%s>%s</option>',
                                    $department['department_id'],
                                    $department['department_name']==$ticket->getDeptName()?' selected ':'',
        ...
        <td width="50%" style="vertical-align: baseline;">
            <table cellspacing="0" cellpadding="4" width="100%" border="0">
                ...
                <tr>
                    ...
                    <?php
                    $sql       = "select staff_id, concat(firstname,' ',lastname) staff_name from sup_staff where isactive=1 order by staff_name;";
                    $res       = db_query($sql);
                    $staff     = db_assoc_array($res);
                    ...
                    <td>
                        ...
                        <span class="inline-edit" data-placement="bottom" data-toggle="tooltip" title="<?php echo __('Update'); ?>">
                        <?php
                        $label    = __('Assigned To');
                        $ticket_id= $ticket->getId();
                        $form_id  = 0;
                        $input_id = 'field_staff_'.$ticket_id;
                        $field_id = $ticket_id;
                        $username = $thisstaff->getUsername();
                        $old_value = $ticket->getAssignee() ? $ticket->getAssignee()->getId() : 0;
                        $ctype    = 'staff';
                        echo '<select id="'.$input_id.'" style="width:190px" onchange="update_ticket_detail(\''.$label.'\','.$ticket_id.','.$form_id.',\''.$input_id.'\','.$field_id.',\''.$username.'\',\''.$old_value.'\',\''.$ctype.'\')">';
                        echo '<option value="0">'.'&mdash; ' . __('Unassigned') .  ' &mdash;'.'</option>';
                        foreach( $staff as $data ){
                            echo sprintf('<option value="%s"%s>%s</option>',
                                $data['staff_id'],
                                ($data['staff_id'] == $old_value) ? ' selected="selected"' : '',
                                $data['staff_name']);
                        }
                        echo sprintf('</select>');
                        ?>
...
<?php
    foreach ($displayed as $a) {
        ...
        $empty_option = '&mdash; ' . __('Empty') .  ' &mdash;';
...
?>
        <tr>
            ...
            <td id="<?php echo sprintf('inline-answer-%s', $field->getId()); ?>">
                ...
                ?>
                <!-- drop down box : only for type choices-->
                <?php //var_dump($field->ht['type']);
                if($field->ht['type']=='choices'){ 
                ?>
                <span class="inline-edit" data-placement="bottom" data-toggle="tooltip" title="<?php echo $title; ?>">
                    <?php
                        // var_dump('$options::',$options,'$html::',$html,'$options[0]::',$options[0]);die();
                        ...
                        $field_id  = $id;
                        $username  = $thisstaff->getUsername();
                        $old_value = $a->getValue().':'.$a->toString();
                        $ctype     = 'choices';
                        echo '<select id="'.$input_id.'" style="width:190px" onchange="update_ticket_detail(\''.$label.'\','.$ticket_id.','.$form_id.',\''.$input_id.'\','.$field_id.',\''.$username.'\',\''.$old_value.'\',\''.$ctype.'\')">';
                        ...
                        foreach( $options as $option ){
                            ...
                            echo sprintf('<option value="%s"%s>%s</option>',
                                Format::striptags($option),
                                $option_data[1]==$v?' selected ':'',
                                $option_data[1]);
                        }
                        echo sprintf('</select>');
                    ?>
                ...
                <?php 
                ...
                else if(in_array($field->ht['type'],array('text','memo'))){
                    $label     = $label;
                    $ticket_id = $ticket->getId();
                    $form_id   = $form->getId();
                    $input_id  = 'field_'.$id;
                    $field_id  = $id;
                    $username  = $thisstaff->getUsername();
                    $old_value = Format::striptags($a->getValue());
                    $ctype     = $field->ht['type'];
                    if($field->ht['type']=='text'){
                        echo '<input type="text" id="'.$input_id.'" value="'.$old_value.'" style="width:380px" onchange="update_ticket_detail(\''.$label.'\','.$ticket_id.','.$form_id.',\''.$input_id.'\','.$field_id.',\''.$username.'\',\''.$old_value.'\',\''.$ctype.'\')">';
                    }
                    if($field->ht['type']=='memo'){
                        $old_value = $a->getValue();
                        echo '<textarea id="'.$input_id.'" class="richtext no-bar small redactor-source">'.$old_value.'</textarea>';
                        if('enable_quick_save_memo'=='')
                            echo '<button onclick="update_ticket_detail(\''.$label.'\','.$ticket_id.','.$form_id.',\''.$input_id.'\','.$field_id.',\''.$username.'\',\''.htmlspecialchars($old_value).'\',\''.$ctype.'\')">Save '.$label.'</button>';
                    }
                }
                // else if($field->ht['type']=='memo'){}
                // else if($field->ht['type']=='datetime'){}
                else{
                ?>
...
function getUrl(tail){
    /* get url and remove the last part */
    var currentUrl      = window.location.href;
    var lastPartIndex   = currentUrl.lastIndexOf('/');
    var updatedUrl      = currentUrl.substring(0, lastPartIndex)+tail;
    return updatedUrl;
}

function update_ticket_detail(label,ticket_id,form_id,input_id,field_id,username,old_value,ctype){

    /* get url and remove the last part */
    var updatedUrl      = getUrl('/tickets_update.php');
    // alert(label+':'+ticket_id+':'+input_id+':'+field_id+':'+username+':'+old_value+':'+ctype)

    /* update ticket detail */
    var new_value = $('#'+input_id).val();
    $.ajax({
        url: updatedUrl, // URL of the server-side script that will handle the update
        type: 'POST', // HTTP method
        data: {
            ticket_id: ticket_id, // Example: ID of the data you want to update
            form_id: form_id,
            field_id: field_id, // Example: Updated name value
            username: username,
            old_value: old_value,
            new_value: new_value,
            ctype: ctype,
        ...
        success: function(response) {
            ...
            $('#msg-txt').html(label+' updated successfully');
            $('#msg-txt').html(label+' updated successfully');
